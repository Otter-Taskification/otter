...
#if !defined(__OTTER_PARALLEL_H__)
#define __OTTER_PARALLEL_H__
 ...
 * @param func The name of the enclosing function.
 * @param line The line in @param file at which this call appears.
 * @return Otter context object which you can/should use for follow-up calls
 */
otter_context_t* otterThreadsBegin(const char* file, const char* func, int line);
 ...
 */
void otterThreadsEnd(otter_context_t* context = NULL);
 ...
 */
otter_context_t* otterTaskBegin(const char* file, const char* func, int line, otter_context_t* parent_context = NULL);
 ...
 * @see `otterTaskBegin()`
 */
void otterTaskEnd(otter_context_t* task_context = NULL);
 ...
 * 
 */
otter_context_t* otterLoopBegin(otter_context_t* parent_context = NULL);
 ...
 * 
 */
void otterLoopEnd(otter_context_t* loop_context = NULL);
 ...
 * 
 */
otter_context_t* otterLoopIterationBegin(otter_context_t* parent_context = NULL);
 ...
 * 
 */
void otterLoopIterationEnd(otter_context_t* loop_context = NULL);
 ...
 * encountering task (`otter_sync_descendants`). If the context argument is
 * NULL (default), then only global is allowed as mode.
 * 
 */
void otter_synchronise(otter_sync_t mode, otter_context_t* context = NULL);


/**
 * Eliminate all pending context objects that are still around.
 */
void otter_context_garbage_collection();
