...
otter_opt_t *tool_setup(tool_callbacks_t *callbacks,
...
static void on_ompt_callback_thread_begin(ompt_thread_t thread_type,
  ...
  case ompt_thread_initial:
    otter_thread_type = otter_thread_initial;
    break;
  case ompt_thread_worker:
    otter_thread_type = otter_thread_worker;
    break;
  case ompt_thread_other:
    otter_thread_type = otter_thread_other;
    break;
  case ompt_thread_unknown:
    otter_thread_type = otter_thread_unknown;
    break;
  default:
    otter_thread_type = otter_thread_unknown;
    break;
 ...
 */
static void on_ompt_callback_thread_end(ompt_data_t *thread) {
  thread_data_t *thread_data = thread->ptr;

  LOG_DEBUG("[t=%lu] (event) thread-end", thread_data->id);
  LOG_DEBUG_IF((thread_data->type == ompt_thread_initial), "final clean-up...");
...
static void on_ompt_callback_parallel_begin(
    ompt_data_t *encountering_task, const ompt_frame_t *encountering_task_frame,
    ompt_data_t *parallel, unsigned int requested_parallelism, int flags,
  ...
  parallel_data_t *parallel_data = new_parallel_data(
      ...
      trace_task_get_id(task_data), task_data, requested_parallelism, flags);
  parallel->ptr = parallel_data;

  /* record enter region event */
  trace_event_enter(thread_data->location,
                    trace_parallel_get_region_def(parallel_data));

  return;
}

static void on_ompt_callback_parallel_end(ompt_data_t *parallel,
 ...
 */
static void
on_ompt_callback_task_create(ompt_data_t *encountering_task,
                             const ompt_frame_t *encountering_task_frame,
                             ompt_data_t *new_task, int flags,
  ...
  task_data_t *task_data =
      new_task_data(thread_data->location, parent_task_region, flags,
                    has_dependences, NULL, codeptr_ra);
  ...
  LOG_DEBUG_TASK_TYPE(thread_data->id,
                      parent_task_data ? trace_task_get_id(parent_task_data)
                                       : 0L,
                      trace_task_get_id(task_data), flags);

  LOG_DEBUG("[t=%lu] END EVENT", thread_data->id);
  return;
}

static void on_ompt_callback_task_schedule(ompt_data_t *prior_task,
  ...
  LOG_DEBUG("[t=%lu] (event) task-schedule %lu (%d) -> %lu", thread_data->id,
            trace_task_get_id(prior_task_data), prior_task_status,
  ...
  case ompt_task_complete:
    otter_prior_task_status = otter_task_complete;
    break;
  case ompt_task_yield:
    otter_prior_task_status = otter_task_yield;
    break;
  case ompt_task_cancel:
    otter_prior_task_status = otter_task_cancel;
    break;
  case ompt_task_detach:
    otter_prior_task_status = otter_task_detach;
    break;
  case ompt_task_early_fulfill:
    otter_prior_task_status = otter_task_early_fulfill;
    break;
  case ompt_task_late_fulfill:
    otter_prior_task_status = otter_task_late_fulfill;
    break;
  case ompt_task_switch:
    otter_prior_task_status = otter_task_switch;
    break;
  case ompt_taskwait_complete:
    otter_prior_task_status = otter_taskwait_complete;
    break;
  default:
    otter_prior_task_status = otter_task_state_undef;
    break;
  }

  trace_event_task_switch(
      thread_data->location, trace_task_get_region_def(prior_task_data),
      otter_prior_task_status, trace_task_get_region_def(next_task_data));
#endif
...
static void on_ompt_callback_implicit_task(ompt_scope_endpoint_t endpoint,
  ...
  LOG_DEBUG("[t=%lu] (event) %s-task-%s", thread_data->id,
  ...
  if (endpoint == ompt_scope_begin) {
    ...
    if (flags & ompt_task_implicit) {
      ...
      encountering_task_region =
          trace_task_get_region_def(encountering_task_data);
    }
    task_data_t *implicit_task_data = new_task_data(
        thread_data->location, encountering_task_region, flags, 0, NULL, NULL);
    task->ptr = implicit_task_data;

    trace_region_def_t *implicit_task_region =
        trace_task_get_region_def(implicit_task_data);
    trace_location_store_region_def(thread_data->location,
                                    implicit_task_region);
    ...
    trace_region_set_task_status(trace_task_get_region_def(implicit_task_data),
                                 otter_task_complete);
...
static void on_ompt_callback_work(ompt_work_t wstype,
                                  ...
                                  ompt_data_t *parallel, ompt_data_t *task,
    ...
    if (endpoint == ompt_scope_begin) {
      trace_region_def_t *wshare_rgn = trace_new_workshare_region(
          ...
          : wstype == ompt_work_taskloop        ? otter_work_taskloop
                                                : 0,
          count, trace_task_get_id(task_data));
...
on_ompt_callback_master(
#endif
    ompt_scope_endpoint_t endpoint, ompt_data_t *parallel, ompt_data_t *task,
    const void *codeptr_ra) {
  thread_data_t *thread_data = (thread_data_t *)get_thread_data()->ptr;
  ...
  LOG_DEBUG("[t=%lu] (event) master-%s", thread_data->id,
            endpoint == ompt_scope_begin ? "begin" : "end");

  if (endpoint == ompt_scope_begin) {
    trace_region_def_t *master_rgn =
        trace_new_master_region(location_id, task_id);
...
static void on_ompt_callback_sync_region(ompt_sync_region_t kind,
  ...
  LOG_DEBUG("[t=%lu] (event) sync-region-%s (%s)", thread_data->id,
            endpoint == ompt_scope_begin ? "begin" : "end",
            kind == ompt_sync_region_barrier            ? "barrier"
            : kind == ompt_sync_region_barrier_implicit ? "implicit barrier"
            : kind == ompt_sync_region_barrier_explicit ? "explicit barrier"
  ...
  if (endpoint == ompt_scope_begin) {
    ...
    ompt_sync_region_barrier_implicit:
      sync_type = otter_sync_region_barrier_implicit;
      break;
    ompt_sync_region_barrier_explicit:
      sync_type = otter_sync_region_barrier_explicit;
      break;
    ompt_sync_region_barrier_implementation:
      sync_type = otter_sync_region_barrier_implementation;
      break;
    ompt_sync_region_taskwait:
      sync_type = otter_sync_region_taskwait;
      break;
    ompt_sync_region_taskgroup:
      sync_type = otter_sync_region_taskgroup;
      break;
    ompt_sync_region_reduction:
      sync_type = otter_sync_region_reduction;
      break;
    ompt_sync_region_barrier_implicit_workshare:
      sync_type = otter_sync_region_barrier_implicit_workshare;
      break;
    ompt_sync_region_barrier_implicit_parallel:
      sync_type = otter_sync_region_barrier_implicit_parallel;
      break;
    ompt_sync_region_barrier_teams:
      sync_type = otter_sync_region_barrier_teams;
      break;
    default:
      sync_type = otter_sync_region_barrier;
    }
    ...
    trace_region_def_t *sync_rgn = trace_new_sync_region(
        sync_type,
        kind == ompt_sync_region_taskgroup ? trace_sync_descendants
                                           : trace_sync_children,
