...
#define LOG_EVENT_CALL(file, func, line, ifunc)                                \
...
void otterTraceInitialise(const char *file, const char *func, const int line) {
  ...
  LOG_EVENT_CALL(src_location.file, src_location.func, src_location.line,
                 __func__);

  // initial task
  task_data_t *initial_task =
      new_task_data(location, NULL, otter_task_initial, 0, &src_location, NULL);

  trace_region_def_t *initial_task_region =
      trace_task_get_region_def(initial_task);
...
void otterTraceFinalise(void) {
  ...
  LOG_DEBUG("writing initial-task region definition from thread queue: %p",
            initial_task_region);
  ...
  */
  ...
  LOG_DEBUG_IF((num_definitions != 0), "definitions in thread queue: %lu",
               num_definitions);
...
void otterThreadsBegin(const char *file, const char *func, const int line) {
  ...
  parallel_data_t *parallel_data = new_parallel_data(
      thread_id, trace_task_get_id(encountering_task), encountering_task, 0, 0);

  trace_region_def_t *parallel_region =
  ...
  task_data_t *implicit_task =
      new_task_data(location, trace_task_get_region_def(encountering_task),
                    otter_task_implicit, 0, &src_location, NULL);

  trace_region_def_t *implicit_task_region =
      trace_task_get_region_def(implicit_task);
...
void otterTaskBegin(const char *file, const char *func, const int line) {
  ...
  task_data_t *encountering_task = get_encountering_task();
  trace_region_def_t *encountering_task_region =
      trace_task_get_region_def(encountering_task);

  task_data_t *task =
      new_task_data(location, encountering_task_region, otter_task_explicit, 0,
                    &src_location, return_address[1]);

  LOG_DEBUG("%lu return_address=%p", trace_task_get_id(task),
            return_address[1]);
  ...
  trace_event_task_switch(location, encountering_task_region, otter_task_switch,
                          task_region);
...
void otterTaskEnd(void) {
  ...
  trace_event_task_switch(location, trace_task_get_region_def(task),
                          otter_task_complete,
                          trace_task_get_region_def(encountering_task));
...
void otterLoopBegin() {
  ...
  trace_region_def_t *loop = trace_new_workshare_region(
      otter_work_loop, 1, trace_task_get_id(encountering_task));
...
void otterSynchroniseTasks(otter_task_sync_t mode) {
  ...
  trace_region_def_t *taskwait = trace_new_sync_region(
      ...
      mode == otter_sync_descendants ? trace_sync_descendants
                                     : trace_sync_children,
...
void otterSynchroniseDescendantTasksBegin() {
  ...
  // LOG_EVENT_CALL(file, func, line, __func__);
  task_data_t *encountering_task = get_encountering_task();
  trace_region_def_t *taskgroup =
      trace_new_sync_region(otter_sync_region_taskgroup, trace_sync_descendants,
                            trace_task_get_id(encountering_task));
...
void otterPhaseBegin(const char *name) {
  ...
  trace_region_def_t *phase = trace_new_phase_region(
      otter_phase_region_generic, trace_task_get_id(encountering_task), name);
