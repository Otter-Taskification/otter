...
def list_vertices(g):
    for v in g.vs:
        print("Node {}:".format(v.index))
        for k, v in v.attributes().items():
            print(f" {k}={v}")

def child_task_ids(tr, task_ids):
    def filter_children(item):
        l, e, a = item
        return a['parent_task_id'] in e.attributes and e.attributes[a['parent_task_id']] in task_ids
    child_ids = {e.attributes[a['unique_id']] for (l,e,a) in filter(filter_children, tr.events)}
    next_ids = child_ids.copy()
    while len(next_ids) != 0:
        next_ids = child_task_ids(tr, child_ids)
        child_ids.update(next_ids)

    return child_ids


def descendant_task_ids(tr, node_id_seq):
    def filter_by_encountering_id(lmap_iter_item):
        l, e, a = lmap_iter_item
        return a['parent_task_id'] in e.attributes and e.attributes[a['parent_task_id']] in node_id_seq
    for (l, e, a) in filter(filter_by_encountering_id, tr.events):
        print(e.attributes[a['unique_id']], e.attributes[a['region_type']])
