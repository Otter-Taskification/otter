...
// Helper functions
void mock_data_destructor(void *ptr) { count_destructor_calls++; }
...
TEST_F(StackTestFxt, SizeNullStackIsZero) { ASSERT_EQ(stack_size(nullptr), 0); }
...
TEST_F(StackTestFxt, NullStackIsEmpty) { ASSERT_TRUE(stack_is_empty(nullptr)); }
...
TEST_F(StackTestFxt, IsCreatedEmpty) { ASSERT_TRUE(stack_is_empty(s1)); }
