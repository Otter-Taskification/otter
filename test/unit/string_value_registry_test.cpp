...
string_registry_callback mock_deleter;
string_registry_callback mock_deleter_cstr;
string_registry_callback mock_deleter_cstr_increments_int_ptr;
string_registry_callback mock_deleter_cstr_checks_called_with_nullptr;
...
protected:
  ...
  void SetUp() override {
    ...
    string_registry_callback *callback = &mock_deleter;
  ...
  void SafeDelete(string_registry *&reg, string_registry_callback *callback,
                  void *data) {
    if (reg) {
      if (callback) {
        string_registry_apply(reg, callback, data);
      }
      string_registry_delete(reg);
      reg = nullptr;
    }
  }
...
void mock_deleter(const char *key, uint32_t value, void *data) { deleted++; }

void mock_deleter_cstr(const char *key, uint32_t label, void *data) {
  std::cerr << "mock_deleter_cstr(" << key << ")" << std::endl;
  deleted++;
}

void mock_deleter_cstr_increments_int_ptr(const char *key, uint32_t label,
                                          void *data) {
...
void mock_deleter_cstr_checks_called_with_nullptr(const char *key,
                                                  uint32_t label, void *data) {
  if (data == nullptr) {
...
TEST_F(TestStringRegistry, IsNonNull) { ASSERT_NE(r, nullptr); }
...
TEST_F(TestStringRegistryDeath, DeathOnNullLabeller) {
  ASSERT_DEATH({ s = string_registry_make(nullptr); }, ".*");
}
...
TEST_F(TestStringRegistry_C, DeleterCalledWithNullData) {
  ...
  const char *key = "foo";
  TestStringRegistry::label_type id = string_registry_insert(t, key);
  TestStringRegistry::SafeDelete(
      t, mock_deleter_cstr_checks_called_with_nullptr, nullptr);
...
TEST_F(TestStringRegistry_C, DeleterCalledWithData) {
  ...
  TestStringRegistry::SafeDelete(t, mock_deleter_cstr_increments_int_ptr,
                                 (void *)&count);
  ASSERT_EQ(count, 3);
}

TEST_F(TestStringRegistry_C, ArgOutlivesScope) {
  auto print_values = [](const char *str, uint32_t label, void *data) -> void {
